...
class downfile(doc):
    ...
    def add_end(self):#添加版记 包括抄送 印发机关 印发日期
        data=self.data
        s=self.s
        zhuson=""
        chaoson=data["抄送机关"]
        ...
        if zhuson!="":
            ...
            self.add_line(s.Range, row_current - 1, 1, 0)
            s.MoveRight()
            s.TypeText("\n")
            if chaoson!="":
                s.TypeText("抄送："+chaoson+"\n")
            s.Text = yinfa + " " * space + date + "印发"
            self.add_line(s.Range, row_current-1+row_zhuson+row_chaoson, 0.7, 0)
            self.add_line(s.Range, row_current +row_zhuson+row_chaoson, 1, 0)
        else:
            if chaoson!="":
                s.Text="抄送："+chaoson
                self.add_line(s.Range, row_current - 1, 1, 0)
                ...
                s.TypeText("\n")
                s.Text = yinfa + " " * space + date + "印发"
                self.add_line( s.Range, row_current-1+row_chaoson, 0.7, 0)
                self.add_line( s.Range, row_current + row_chaoson, 1, 0)
            else:
                s.Text = yinfa + " " * space + date + "印发"
                self.add_line(s.Range, row_current - 1, 1, 0)
                self.add_line( s.Range, row_current , 1, 0)
