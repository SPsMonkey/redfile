...
def setFont(fontName="仿宋",size="三号",color="黑色"):
    ...
    if type(size)==type(1) or type(size)==type(1.5):
        ...
        Selection.font.size=font_size[size]
    if color == "黑色":
...
def getPosY():#获取选择点距离上页面上边缘的磅数
    return Selection.Information(6)  #5就是x

def add_line(anchor,n,weight,color): #在文档中添加横线
    line_height=(33+10.5*n)*2.835 # 442.5/156
    line=docx.Shapes.AddLine(79, line_height, 521.5, line_height,anchor).line
    line.Weight=weight
    line.ForeColor.RGB=color

def drawTheRedLine(height,anchor,weight=3):
    if anchor==False:
        shape= docx.Shapes.AddLine(79, height, 521.5, height)
        line=shape.line
        shape.RelativeHorizontalPosition=1
        shape.RelativeVerticalPosition = 1
    else:
        line = docx.Shapes.AddLine(79.38, height, 521.64, height, anchor).line
    line.Weight = weight
...
def setPage():#页面页面字号设置
    ...
    # 是经过实验的，可以看看公文标准的图示
    # 版心指的是文字与边缘距离
    docx.PageSetup.TopMargin = 3.3 * cm_to_points
    # 上边距3.3厘米
    docx.PageSetup.BottomMargin = 3.3 * cm_to_points
    # 下边距3.3厘米
    docx.PageSetup.LeftMargin = 2.8 * cm_to_points
    # 左边距2.8厘米
    docx.PageSetup.RightMargin = 2.6 * cm_to_points
    # 右边距2.6厘米
    ...
    docx.PageSetup.LayoutMode = 1
    # 指定行和字符网格
    docx.PageSetup.CharsLine = 28
    # 每行28个字
    docx.PageSetup.LinesPage = 22
    # 每页22行，会自动设置行间距

    # 页码设置
    docx.PageSetup.FooterDistance = 2.4 * cm_to_points
    # 页码距下边缘2.8厘米
    docx.PageSetup.DifferentFirstPageHeaderFooter = 0
    # 首页页码相同
    docx.PageSetup.OddAndEvenPagesHeaderFooter = 0
    # 页脚奇偶页相同
    w = docx.windows(1)
    # 获得文档的第一个窗口
    w.view.seekview = 4
    ...
    # 页码向右缩进1字符（21磅）
    docx.Styles('页眉').ParagraphFormat.Borders(-3).LineStyle = 0
    # 页眉无底边框横线
